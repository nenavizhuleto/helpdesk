...
<script lang="ts">
  ...
  import { Button, Badge, Alert, Label, Input, InputAddon, ButtonGroup, Checkbox } from "flowbite-svelte";

	export let data: PageData;
	// TODO: Why we expect that task cannot be undefined?
	let task = data?.task!;

  let range = [1, 1, 1, 1, 1, 1, 1,]
</script>
    ...
    <div class="flex flex-col flex-grow w-full bg-white overflow-hidden">
      ...
      <div class="flex flex-col flex-grow h-0 p-4 overflow-auto">
        ...
        <div class="flex w-full mt-2 space-x-3 max-w-xs ml-auto justify-end">
          <div>
            <!-- Message Text-->
            <div class="bg-blue-600 text-white p-3 rounded-l-lg rounded-br-lg text-sm">
      ...
      <div class="p-6 border-t border-gray-300">
        <ButtonGroup class="w-full">
          <Input id="input-addon" size="lg" type="text" placeholder="Ваше сообщение..." />
          <Button color="blue">Отправить</Button>
        </ButtonGroup>
      </div>
    </div>
  </div>
  <div class="w-full max-w-xs h-screen  flex flex-col border-l border-gray-300 bg-white transition-all duration-300">
    <div class="px-6 py-6 border-b border-gray-300">
